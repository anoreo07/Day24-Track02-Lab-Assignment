...
def validate_anonymized_data(filepath: str) -> dict:
    ...
    for val in df["cccd"]:
        if str(val).isdigit() and len(str(val)) == 12:
            results["failed_checks"].append("Raw CCCD found in anonymized data")
            results["success"] = False
            break

    # Check 2: Không có null values trong các cột quan trọng
    important_cols = ["patient_id", "ho_ten", "benh"]
    ...
    original_count = len(pd.read_csv("data/raw/patients_raw.csv"))
